...
export default function BannerCard({ banner, index, onEdit, onDelete, onToggle }) {
  return (
    <div className={`bg-white border border-[#e0e0ff] rounded-2xl p-4 flex items-center gap-4 transition${banner.active ? "border-[#e0e0ff]" : "border-gray-200 opacity-60"}`} >
      <MdOutlineDragIndicator size={20} className="text-gray-300 shrink-0 cursor-grab" />
      <div
        className="w-20 h-14 rounded-xl shrink-0 flex items-center justify-center overflow-hidden"
        style={{ backgroundColor: banner.bgColor }}
      >
        {banner.image ? (
          <img src={banner.image} alt="banner thumbnail" className="w-full h-full object-cover" />
        ) : (
          <MdOutlineImage size={22} className="text-white/60" />
        )}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[10px] bg-[#f0f0ff] text-[#6C63FF] px-2 py-0.5 rounded-full font-medium">
