    ...
    <div className="p-4 sm:p-6 min-h-screen bg-admin-bg">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-danger hover:bg-red-600 text-white text-sm font-medium rounded-xl transition shadow-sm"
        >
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-3 mb-5">
        <div className="flex items-center gap-2 bg-admin-card border border-admin-border rounded-xl px-3 py-2 w-full sm:w-64">
      ...
      <div className="md:hidden flex flex-col gap-3 mb-4">
        {filtered.length === 0 ? (
          <div className="bg-admin-card rounded-2xl border border-admin-border p-10 text-center">
          ...
          filtered.map((product) => (
            <div key={product.id} className="bg-admin-card border border-admin-border rounded-2xl p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 pt-1 border-t border-admin-border">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-admin-border text-sm text-gray-500 hover:border-primary hover:text-primary hover:bg-primary-light transition"
                >
                  <MdOutlineEdit size={16} /> Edit
                </button>
                <button
                  onClick={() => setDeleteTarget(product)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-admin-border text-sm text-gray-500 hover:border-danger hover:text-danger hover:bg-danger-light transition"
                >
      ...
      <div className="hidden md:block bg-admin-card border border-admin-border rounded-2xl overflow-x-auto">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              filtered.map((product, idx) => (
                <tr
                  key={product.id}
                  className={`border-b border-admin-border hover:bg-primary-light transition ${idx === filtered.length - 1 ? "border-b-0" : ""}`}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img src={product.image} alt={product.name} className="w-10 h-10 rounded-lg object-cover border border-admin-border shrink-0" />
