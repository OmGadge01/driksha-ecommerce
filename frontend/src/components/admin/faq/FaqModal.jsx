...
const inputCls =
  "border border-admin-border rounded-xl px-3 py-2.5 text-sm text-gray-700 " +
...
export default function FaqModal({ faq, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-3 md:px-4">
      <div className="bg-admin-card rounded-2xl shadow-xl w-full max-w-lg flex flex-col gap-4 md:gap-5 p-4 md:p-6 max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-sm md:text-base font-semibold text-gray-800">
            {faq.id ? "Edit FAQ" : "Add New FAQ"}
          </h2>
          <button onClick={onClose} className="p-1.5 hover:bg-admin-bg rounded-xl transition text-gray-400">
        ...
        <div className="flex items-center justify-between p-3 bg-admin-bg rounded-xl border border-admin-border">
          ...
          >
            <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-200 ${form.active ? "left-5" : "left-0.5"}`} />
          </button>
        </div>
        <button
          onClick={() => isValid && onSave(form)}
          disabled={!isValid}
          className="flex items-center justify-center gap-2 w-full py-2.5 bg-primary hover:bg-primary-dark text-white text-sm font-medium rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
